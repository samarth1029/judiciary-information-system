...
class Register:
    def __init__(self, root):
        ...
        self.studentuser_entry = ttk.Entry(main_frame, textvariable=self.var_fname, width=37,
        ...
        self.contact_entry = ttk.Entry(main_frame, textvariable=self.var_contact, width=37,
                                       font=("times new roman", 10))
        self.contact_entry.place(x=20, y=160)

        search_label = Label(main_frame, text="Select Security Question", font=("lucida handwriting", 10, "bold"),
                             bg='lightblue')
        search_label.place(x=20, y=190)
        search_combo = ttk.Combobox(main_frame, textvariable=self.var_securityQ, font=("times new roman", 10), width=35,
                                    state="readonly")
        ...
        self.pass_entry = ttk.Entry(main_frame, textvariable=self.var_pass, width=37, font=("times new roman", 10),
                                    show='*')
        ...
        confirm_lbl = Label(main_frame, text="Confirm Password", font=("lucida handwriting", 10, "bold"),
                            bg='lightblue')
        confirm_lbl.place(x=350, y=250)
        self.contact_entry = ttk.Entry(main_frame, textvariable=self.var_confirmpass, width=37,
                                       font=("times new roman", 10), show="*")
        ...
        checkbtn2_label = Label(main_frame, text="I agree to the terms and conditions", font=("times new roman", 12),
                                bg="lightblue")
        ...
        b1 = Button(main_frame, image=self.photoimg2, command=self.register, borderwidth=0, cursor="hand2",
                    font=("times new roman", 12), fg='lightblue')
        b1.place(x=40, y=350, width=150)

        img3 = Image.open(r"Images\login.jpg")
        img3 = img3.resize((150, 30), Image.ANTIALIAS)
        self.photoimg3 = ImageTk.PhotoImage(img3)
        b3 = Button(main_frame, image=self.photoimg3, borderwidth=0, cursor="hand2", command=self.return_login,
                    font=("times new roman", 12), fg='lightblue')
    ...
    def register(self):
        if self.var_fname.get() == "" or self.var_pass.get() == "" or self.var_confirmpass.get() == "" or self.var_securityQ.get() == "Select" or self.var_securityA.get() == "":
            ...
            connection = mysql.connector.connect(host='localhost', database='face_recogniser', user='root',
                                                 password='samarth29rps123456')
