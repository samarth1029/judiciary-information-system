...
class LoginWindow:
    def __init__(self, root):
        ...
        self.user_entry = ttk.Entry(self.root2, textvariable=self.username, width=37,
        ...
        self.pass_entry = ttk.Entry(self.root2, textvariable=self.password, width=37, font=("times new roman", 10),
                                    show='*')
        self.pass_entry.place(x=20, y=195)

        login_btn = Button(self.root2, text="Login", command=self.login, width=14, font=("times new roman", 13),
    ...
    def login(self):
        if self.user_entry.get == "" or self.pass_entry.get() == "":
            ...
            _dict = {
                "query": f'''
                            SELECT * FROM `sandbox-381608.registration.registered_users`
                            WHERE 
